...
ZelzalDV_cmd = (
    "[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗕𝗗𝗧𝗛𝗢𝗡 - اوامــر المطـور المســاعد](t.me/BDthon) .\n\n"
...
async def add_sudo_user(event):
    ...
    output += "**⎉╎يتم الان اعـادة تشغيـل بـوت بــــــدثــون انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your BDthon__.\n"
                )
            else:
    ...
    output = (
        result + "**⎉╎يتم الان اعـادة تشغيـل بـوت بــــــدثــون انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
